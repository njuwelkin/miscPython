    ...
    def _ajust_item_score(self):
        for i in range(len(self.dest)):
            if self.dest[i].name == 'red_gem':
                ...
                self.dest[i].score = 1.2
            else:
                self.dest[i].score = 0.1


        p0 = self.context.players[0]
        count_item = p0.item_count
        if count_item['red_gem'] <= count_item['blue_gem'] and count_item['red_gem'] <= count_item['yellow_gem']:
            return
        if self.context.round < 300:
            return

        gap_space = count_item['red_gem'] - count_item['blue_gem']
        yellow_gap = count_item['red_gem'] - count_item['yellow_gem']
        for i in range(len(self.dest)):
            if self.dest[i].name == 'blue_gem':
                if gap_space <= 0:
                    continue
                if self.context.round < 300:
                    continue
                elif self.context.round < 400:
                    self.dest[i].score += gap_space * 0.1
                elif self.context.round < 500:
                    self.dest[i].score += gap_space * 0.15
                elif self.context.round < 700:
                    self.dest[i].score += gap_space * 0.25
                elif self.context.round < 800:
                    self.dest[i].score += gap_space * 0.3
                elif self.context.round < 900:
                    self.dest[i].score += gap_space * 0.4
                else:
                    self.dest[i].score += gap_space * 1
            elif self.dest[i].name == 'yellow_gem':
                if yellow_gap <= 0:
                    continue
                if yellow_gap <= gap_space:
                    continue
                if self.context.round < 700:
                    return
                elif self.context.round < 800:
                    self.dest[i].score += yellow_gap / 10
                elif self.context.round < 900:
                    self.dest[i].score += yellow_gap / 5
                else:
                    self.dest[i].score += yellow_gap / 2
            print(self.dest)
