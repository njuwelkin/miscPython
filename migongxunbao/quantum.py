...
class Solution:
    ...
    def dfs(self, next_dest, visited, remain_steps=[0, 0], depth=0):
        l0 = len(next_dest[0])
        l1 = len(next_dest[1])
        
        if l0 == 0 and l1 == 0:
            return [remain_steps[0]/4, remain_steps[1]/4], [f"{len(self.dest)-1}", f"{len(self.dest)-2}"]
        close_one = 0
        if l0 != 0 and l1 != 0:
        ...
        r_s = (copy.copy(remain_steps))
        r_s[close_one] = 0
        r_s[1-close_one] += steps
        res1, path1 = self.dfs(new_next, new_visited, remain_steps=r_s, depth=depth+1) # 递归
        ...
        res2, path2 = self.dfs(new_next, new_visited, remain_steps=remain_steps, depth=depth+1)
